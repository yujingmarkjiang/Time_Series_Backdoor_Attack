...
class Classifier_RESNET:
    ...
    def fit(self, x_train, y_train, x_val, y_val, y_true, nb_epochs=50):
    ...
    def fit_backdoor(self, x_train, y_train, x_val, y_val,
                     ...
                     clean_label=True, batch_size=64, nb_epochs=50):
        if not tf.test.is_gpu_available:
            print('error')
            exit()
        # x_val and y_val are only used to monitor the test loss and NOT for training
        ...
        for e in range(500):
            x_train_backdoor, y_train_backdoor = pattern_generator(x_train, y_train, y_target,
                                                                   ...
                                                                   one_hot=True)
            print("Epoch:", e + 1)
            hist = self.model.fit(x_train_backdoor, y_train_backdoor, batch_size=mini_batch_size, epochs=nb_epochs,
        ...
        y_pred_backdoor = np.argmax(self.model.predict(x_val_backdoor), axis=1)

        df_metrics = save_logs(self.output_directory, hist, y_pred_classlabel, y_val_classlabel, duration,
