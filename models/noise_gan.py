...
class Classifier_Noise_GAN:
    ...
    def _fit_backdoor(self, backdoor_clf, x_train, y_train, x_test, y_test, y_target, y_test_classlabel,
        ...
        # mm = keras.models.Model(inputs=self.model.input, outputs=self.model.outputs[0])
        # xx = x_train[0:1,:,:]
        # np.save('sample1.npy', xx)
        # np.save('pattern1.npy', xx * mm(xx))
        for e in range(1, 50):
            print("Epoch:", e)
            '''
            ...
            #K.set_value(full_model.optimizer.learning_rate, 0.001)

            full_model.fit(x_train_backdoor, y_train_backdoor, batch_size=4, epochs=20)

            # Train backdoor classifier
            full_model = self.get_full_model(backdoor_clf, False, True)

            full_model.fit(x_train_backdoor, y_train_backdoor, batch_size=16, epochs=5)
            ...
            #K.set_value(backdoor_clf.model.optimizer.learning_rate, 0.005)
    ...
    def fit(self, x_train, y_train, x_test, y_test, y_test_classlabel, backdoor_clf, process_instances,
        ...
        print("Pre-training...")
        backdoor_clf.fit(x_train, y_train, x_test, y_test, y_test_classlabel, nb_epochs=10)
